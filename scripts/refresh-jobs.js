...
async function fetchGreenhouse(name, slug) {
  ...
  return d.jobs.map(function(j) {
    return { job_id: "gh_" + j.id, job_title: j.title, employer_name: name, job_apply_link: j.absolute_url, job_description: trimDesc(j.content), job_posted_at: j.updated_at, _company: name, _loc: j.location ? j.location.name : "" };
  });
}
...
async function fetchJSearch(name) {
  ...
  return d.data.filter(function(j) {
    ...
    return { job_id: j.job_id, job_title: j.job_title, employer_name: j.employer_name, employer_logo: j.employer_logo, job_apply_link: j.job_apply_link, job_description: trimDesc(j.job_description), job_employment_type: j.job_employment_type || null, job_min_salary: j.job_min_salary || null, job_max_salary: j.job_max_salary || null, job_posted_at: j.job_posted_at_datetime_utc || null, _company: name, _loc: [j.job_city, j.job_state, j.job_country].filter(Boolean).join(", ") };
  });
}
